...
function handleBotTurn(io, room) {
    ...
    setTimeout(() => {
        if (room.interaction) {
            ...
            if (chosenButton) {
                handleAction(io, room, activePlayer, chosenButton.actionId, chosenButton.actionPayload);
            }
            return;
        }
...
function handleAction(io, room, actor, actionId, payload) {
    ...
    if (actionId === 'auctionProperty') {
        clearInteraction(room);
        if (startAuction(room, activePlayer, typeof payload === 'number' ? payload : activePlayer.position)) {
            logMsg(room, `${activePlayer.name} sends ${BOARD_SPACES[payload].name} to auction.`, activePlayer.color);
            offerAuctionTurn(room);
            broadcast(io, room.code);
        } else {
            continueAfterResolution(io, room, activePlayer);
        }
